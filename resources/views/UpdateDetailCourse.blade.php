...
<script>
        ...
        for(var k=0;k<precourseworks.length;k++){
            for(var j=0;j<presubworks.length;j++){
                if(presubworks[j]['CourseworkId'] == precourseworks[k]['CourseworkId']) {
                    ...
                    content += '<input class="form-control" value="'+presubworks[j]['Content']+'" type="search" id="week_sub_input'+sub_click[index]+'_'+index+'">';
                    content += '</div>';
                    content += '<a href="javascript:void(0);" onclick="MinusSub('+sub_click[index]+','+index+')"><i class="icon-copy ion-minus-circled"></i></a>';
                    content += '</div>';
    ...
    function UpdateCurriculum(_courseid) {
        var error_flag = 0;
        for(var i=1;i<=week;i++){
                ...
                if(course_input_value == "") {
                    ...
                    for(j=1;j<sub_click[i];j++){
                        ...
                        if(sub_course_input_value == "") {
                            $("#modal-content4").empty();
                            $("#modal-content4").append('<p>소주제 내용을 채워주세요</p>');
                            $("#alert-modal").modal('show');
                            error_flag = 1;
                            break;
                        } else {
    ...
    <div id="overviews" class="section wb">
        <div class="container">
            ...
            <div>
                <div class="row">
                    <div class="col-6">
                        <button id="payment-button" class="btn btn-lg btn-info btn-block" onclick="UpdateCurriculum({{ $course->CourseId }})">
